      ...
      <div className="w-[300px] bg-bg-panel border-l border-border-muted flex flex-col p-6 h-full overflow-y-auto relative text-text-main">
        ...
        <div className="flex flex-col gap-3 mb-8 pb-8 border-b border-border-muted">
          ...
          {selectedItem.id !== "root" && (
            <button
              className="flex items-center gap-3 p-2 border border-border-muted rounded-lg font-medium text-sm transition-all bg-bg-main hover:bg-red-500/10 hover:border-red-500/50 hover:text-red-500 cursor-pointer"
        ...
        <div className="flex flex-col gap-6">
          <div className="flex gap-4 items-start text-text-muted">
            ...
            <div className="flex flex-col gap-1">
              <span className="text-xs font-normal uppercase tracking-wider text-text-muted/60">
                Type
              </span>
              <span className="text-sm text-text-main">
                {isFolder ? "Folder" : selectedItem.type}
              </span>
            </div>
          </div>
          ...
          {!isFolder && (
            <div className="flex gap-4 items-start text-text-muted">
              ...
              <div className="flex flex-col gap-1">
                ...
                <span className="text-sm text-text-main">
                  {selectedItem.size ? `${(selectedItem.size / 1024).toFixed(2)} KB` : "Unknown"}
                </span>
              </div>
            </div>
          )}
          ...
          <div className="bg-bg-panel border border-border-muted rounded-xl w-full max-w-[400px] p-6 shadow-2xl">
            ...
            <div className="flex justify-end gap-4">
              <button
                ...
                className="py-2 px-4 rounded-lg font-medium text-sm transition-all bg-bg-hover text-text-main hover:bg-bg-hover/80 cursor-pointer"
              >
                Cancel
              </button>
              <button
                onClick={handleDelete}
                className="py-2 px-4 rounded-lg font-medium text-sm transition-all bg-red-500 text-white hover:bg-red-600 cursor-pointer"
              >
                Delete
              </button>
            </div>
