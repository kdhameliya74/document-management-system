...
jest.mock("@/shared/components/common/ActivityLog", () => () => (
  <div data-testid="activity-log">Activity Log</div>
));
jest.mock("@/shared/components/common/UserTag", () => ({ collaborator }) => (
  <div>{collaborator?.name || collaborator?.fullName}</div>
));
...
describe("RightSidebar", () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(documentActions, "summarizeDocument").mockImplementation(() => () => {
      return Promise.resolve({ unwrap: () => Promise.resolve() });
    });
    jest
      .spyOn(documentActions, "setShowDetails")
      .mockImplementation((val) => ({ type: "documentSystem/setShowDetails", payload: val }));
    jest
      .spyOn(documentActions, "setActiveModal")
      .mockImplementation((val) => ({ type: "documentSystem/setActiveModal", payload: val }));
    jest
      .spyOn(documentActions, "setModalProps")
      .mockImplementation((val) => ({ type: "documentSystem/setModalProps", payload: val }));
    jest
      .spyOn(documentActions, "clearUISelection")
      .mockImplementation(() => ({ type: "documentSystem/clearUISelection" }));
  });
