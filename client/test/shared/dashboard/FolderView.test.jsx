...
jest.mock("@/shared/components/dashboard/FolderItem", () => ({ folder, onNavigate }) => (
    <div data-testid="folder-item" onClick={() => onNavigate(folder.id)}>{folder.name}</div>
));
jest.mock("@/shared/components/dashboard/FileItem", () => ({ file, onSelect }) => (
    <div data-testid="file-item" onClick={() => onSelect(file.id)}>{file.name}</div>
));
...
jest.mock("@/shared/components/common/Filters", () => () => <div data-testid="filters">Filters</div>);
